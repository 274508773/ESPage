 ...
 * Generally, events allow you to execute some function when something happens with an object (e.g. the user clicks on the map, causing the map to fire `'click'` event).
 *
 * @example
 *
 * ```js
 ...
 * ```
 *
 *  deals with event listeners by reference, so if you want to add a listener and then remove it, define it as a function:
 *
 * ```js
 * function onClick(e) { ... }
 *
...
ES.Evented = ES.Class.extend({
	 ...
	 * Note that if you passed a custom context to `on`,
	 * you must pass the same context to `off` in order to remove the listener.
	 *
	 * @alternative
	...
	_off: function (type, fn, context) {
		...
		if (!fn) {
			...
			for (i = 0, len = listeners.length; i < len; i++) {
				listeners[i].fn = ES.Util.falseFn;
			}
